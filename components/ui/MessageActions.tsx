...
export function EditAlert () {
    ...
    const handleEdit = async () => {
        ...
        if (edittedMessage !== "") {
            const newMessage = {...actionMessage, text: edittedMessage, is_edit: true}
            optimisticUpdateMessage(newMessage as Imessage)
            const {error} = await supabase.from("messages").update({text: edittedMessage, is_edit: true}).eq("id", actionMessage?.id!)
            if (error) {
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                <Textarea onChange={handleChange} defaultValue={actionMessage?.text} ref={messageRef} />
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Cancel</AlertDialogCancel>
              <AlertDialogAction disabled={disabled} onClick={handleEdit}>Confirm</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      );
